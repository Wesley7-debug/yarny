...
const ActionButton = ({ onClick, icon, bg, hover, text }) => (
  <button
    onClick={onClick}
    className={`w-16 h-16 ${bg} ${hover} ${text} text-2xl font-bold rounded-full flex items-center justify-center transition-transform transform hover:scale-110`}
  >
    {icon}
  </button>
);
...
const MatchingHomeTab = () => {
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center px-4 py-10">
      <div
        className={`relative w-full max-w-md bg-white rounded-2xl shadow-xl overflow-hidden transition-all duration-300 ${animation}`}
      >
        {/* Profile Image */}
        <img
          src={profile.img}
