...
// const StatusPost = ({ setShowStatusPost }) => {
//   const { uploadStatus, isUploadingStatus } = useStatusStore();
...
//               navigate("/", { state: { goTStatus: true } }),
//               setSelectedGroup(null),
//               setSelectedUser(null);
//           }}
//           className="bg-white bg-opacity-50 p-2 rounded-full text-black cursor-pointer z-4"
//         >
//           <ArrowLeft size={20} />
//         </button>
//       </div>
...
const StatusPost = ({ setShowStatusPost }) => {
  const { uploadStatus } = useStatusStore();
  const { setSelectedUser } = messageStore();
  const { setSelectedGroup } = useGroupStore();
  const [textStatus, setTextStatus] = useState("");
  const [bgColor, setBgColor] = useState(COLORS[3]);
  ...
  const handleMediaChange = (e) => {
    ...
    const newMedia = valid.map((file) => ({
      file,
      previewURL: URL.createObjectURL(file),
      caption: "", // ➕ Add caption field to each media
    }));
    ...
    >
      ...
      {mediaFiles.length > 0 && (
        <div className="relative flex-grow h-full w-full bg-black">
          ...
          {/* Caption Input */}
          <div className="absolute bottom-4 left-0 right-0 z-40 rounded-3xl bg-black/20 text-purple-800 ring-1 ring-purple-300 p-2.5 max-w-3xl mx-auto">
            <form
              onSubmit={(e) => {
                e.preventDefault();
                handleSubmit();
            ...
            >
              <textarea
                rows={1}
                placeholder="Add a caption..."
                value={mediaFiles[currentPreviewIndex]?.caption || ""}
                onChange={(e) => {
                  const updated = [...mediaFiles];
                  updated[currentPreviewIndex].caption = e.target.value;
